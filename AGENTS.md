...
`SafeFAME-TS_v2_支撑材料.zip` 及其 SHA256 侧文件是按需生成的本地交付物，不纳入 Git，
也不再在 `paper/final` 保存重复副本。需要提交支撑材料时运行 `src/package_release.py`，
核验器在本地包存在时严格检查；普通克隆中不存在时应明确跳过而非失败。

除非用户明确要求，不要新增 Git LFS 文件，也不要把 ZIP、虚拟环境、缓存或可重建的大型证据上传到仓库。
