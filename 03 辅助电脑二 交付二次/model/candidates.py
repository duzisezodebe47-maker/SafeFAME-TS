...
主控任务书正文写「α 候选沿用仓库 `RIDGE_ALPHAS`」（9 档，含 1e-4/1e-3），
但 `split_spec_v2.json` 的 `alpha_grid` 只有 **7 档**（0.01 ~ 10000）。
本模块以**机器可读的协议文件**为准，并把该冲突记录在 `RUNBOOK.md`。
"""
...
# 协议冻结的 7 档网格（split_spec_v2.json: alpha_grid）
