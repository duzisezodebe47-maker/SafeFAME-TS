...
class FrozenBundle:
    ...
    manifest: dict
    # 测试隔离模式与证据（第七轮 nan-mask / 第八轮 strict）
...
def task_seasonal_period(spec_path: Path, task_id: str) -> int:
    """从**冻结 spec 顶层**的 `seasonal_periods` 取本任务的季节周期（第六轮修复 3）。

    冻结 spec 的实际结构是**顶层**映射，不在任务登记里::

        "seasonal_periods": {"Agriculture": 12, "Climate": 52,
                             "SocialGood": 12, "Environment": 7}

    **不能默认 12** —— Climate 是 52、Environment 是 7，默认值对这两个任务是错的，
    而 `SeasonalNaive` 基线会因此与主控选出不同的预测。
    """
    spec = _read_json(Path(spec_path))
    match = TASK_ID_RE.match(task_id)
    if match is None:
        raise BundleUnavailable(f"task_id 格式不符: {task_id}")
    periods = spec.get("seasonal_periods")
    if not isinstance(periods, dict):
        raise BundleUnavailable("冻结 spec 未声明 seasonal_periods")
    domain = match.group("domain")
    if domain not in periods:
        raise BundleUnavailable(
            f"冻结 spec 的 seasonal_periods 未登记 {domain}（已登记: {sorted(periods)}）")
    value = int(periods[domain])
    if value < 1:
        raise BundleUnavailable(f"{domain} 的注册季节周期非法: {value}")
    return value
...
TARGET_KEYS = ("targets", "targets_standardized")
TEST_TRUTH_FILE_NAMES = ("targets.npy", "targets_standardized.npy", "targets_raw.npy",
                         "target_time.npy")


def _load_targets_strict(path: Path, test_mask: np.ndarray) -> np.ndarray:
    """**严格隔离**：只把非 test 行读进内存，test 行永不 materialize。

    做法：`np.load(..., mmap_mode="r")` 拿到视图（不触发整表读取），
    再用布尔索引**只取非 test 行**写进新数组；test 行填 NaN 占位。
    与"整表读进来再把 test 行改掉"的区别是可验证的：本函数从不整表 np.load，
    审计据此判"test 真值未被当作数据读取"（第七轮任务书 §五）。
    """
    view = np.load(path, mmap_mode="r")           # 只映射，不读页
    if view.shape[0] != int(test_mask.size):
        raise BundleUnavailable(
            f"{path.name} 行数 {view.shape[0]} 与 samples 行数 {test_mask.size} 不符")
    out = np.empty(view.shape, dtype=view.dtype)
    keep = ~np.asarray(test_mask)
    out[keep] = view[keep]                        # 只索引非 test 行
    out[~keep] = np.nan
    return out


def _load_direct(bundle_folder: Path, task_id: str, scenario: str,
                 isolate_strict: bool = False):
    """官方适配层：按 `data_bundle_required` 直读，不依赖数据侧 `bundle.py`。

    数据侧 `bundle.py` 在孤儿分支上、不一定随仓库分发，所以必须有这条路径。
    字段缺失一律硬失败 —— 不代填、不静默跳过。

    `isolate_strict=True`：`targets*` 走 `_load_targets_strict`（test 行不 materialize）。
    """
    ...
    test_mask = (samples["segment"].to_numpy() == "test")
    ...
    for key in shared:
        ...
        if not path.is_file():
            ...
            continue
        if isolate_strict and key in TARGET_KEYS:
            arrays[key] = _load_targets_strict(path, test_mask)
        else:
...
def _load_via_data_side(bundle_folder: Path, task_id: str, scenario: str,
                        signature_value: str, isolate_strict: bool = False):
    if isolate_strict:
        # 严格隔离必须走直读：数据侧 `bundle.py` 的内部加载方式不受本函数控制，
        # 无法保证"test 行不被 materialize"。直读路径同样经过 verify_bundle 的
        # 签名与逐文件哈希校验，故不降低完整性强度。
        print("NOTE: 严格隔离模式使用官方适配层直读（绕过数据侧 bundle.py 的加载实现）；"
              "签名与逐文件哈希校验仍由 verify_bundle 执行。", file=sys.stderr)
        return _load_direct(bundle_folder, task_id, scenario, isolate_strict=True)
...
def read_frozen_bundle(
    ...
    `isolate_test` 三档：
      ...
      `False`      不隔离（历史行为）
      `True`       载入后把 test 段真值置 NaN（第七轮；test 行**被读过**再抹掉）
      `"strict"`   test 行**从不 materialize**（第八轮）：`targets*` 用
                   `np.load(mmap_mode="r")` 只布尔索引非 test 行，test 行以 NaN 占位。
                   审计据此判"test 真值未被当作数据读取"。

    为什么不是"不打开 `targets*.npy`"：该文件是**单表含全部段**（train/cal/dec/test
    同行），清单完整性哈希与 train/cal/dec 拟合都必须访问它。`"strict"` 是能做到的
    最强保证：整表从不读入，只索引允许的行。
    """
    folder = Path(bundle_folder)
    ...
    strict = isolate_test == "strict"
    samples, arrays, _ = _load_via_data_side(folder, task_id, scenario, expected_signature,
                                            isolate_strict=strict)
    arrays = {k: np.asarray(v) for k, v in arrays.items()}
    if strict:
        isolation = {"mode": "strict",
                     "test_rows_materialized": False,
                     "mechanism": "np.load(mmap_mode='r') + 只布尔索引非 test 行；"
                                  "test 行以 NaN 占位，从未整表读入"}
        samples = samples  # 不变
    elif isolate_test:
        arrays = _null_test_targets(arrays, samples, task_id)
        isolation = {"mode": "nan-mask", "test_rows_materialized": True,
                     "mechanism": "整表载入后把 test 段行置 NaN"}
    else:
        isolation = {"mode": "none", "test_rows_materialized": True, "mechanism": None}
