...
def to_inference_bundle(part: dict[str, np.ndarray]) -> FeatureBundle:
    """构建**仅含特征**的推理视图（第五轮 A.5）。

    测试段预测不得把 `targets` / `targets_standardized` 交给模型 —— 不能只在
    注释里声称"未使用测试真值"，而要让它**根本拿不到**。
    评分由主控另一进程读取真值完成。
    """
    required = ("numeric_history", "semantic", "quality", "text_available", "origin_index")
    missing = [k for k in required if k not in part]
    if missing:
        raise BundleUnavailable(f"推理视图缺少必需特征字段: {missing}")
    return FeatureBundle(
        numeric_history=part["numeric_history"],
        semantic=part["semantic"],
        quality=part["quality"],
        text_available=part["text_available"],
        origin_index=part["origin_index"],
        targets=None,                  # ← 测试真值不传入
        targets_standardized=None,
    )
