...
from bundle_reader import (  # noqa: E402
    HEX64, BundleUnavailable, assert_grid, read_frozen_bundle, segment_bounds,
    sha256_file, task_seasonal_period,
)
...
def check_route(
    ...
    *, bundle_signature: str, split_spec_sha256: str,
    ...
    """
    # 第六轮修复 1：只接受**主控封印**的路由。
    # 主控 `route_seal.seal_route` 会写入 status/scenario/两锚/两个来源哈希；
    # 仅有"两个锚"的自造路由**不足以**解锁测试预测。
    sealed_required = ("status", "scenario", "split_spec_sha256", "bundle_signature",
                       "selection_manifest_sha256", "refit_review_sha256",
                       "inputs_sha256", "task_id", "fold_id", "selected", "fallback",
                       "selection_data_segments")
    missing_fields = [k for k in sealed_required if k not in route]
    if missing_fields:
        raise RouteRejected(
            f"路由缺少主控封印字段: {missing_fields} —— "
            f"自造路由不能解锁测试预测，必须来自主控 seal_route.py")
    if str(route["status"]) != "frozen":
        raise RouteRejected(
            f"路由的 status={route['status']!r} 不是 'frozen'；"
            f"只有主控封印后的路由才能执行测试预测")
    for key in ("selection_manifest_sha256", "refit_review_sha256", "inputs_sha256"):
        value = route[key]
        if isinstance(value, dict):
            if not value or any(not HEX64.match(str(v)) for v in value.values()):
                raise RouteRejected(f"路由的 {key} 含非法哈希")
        elif not HEX64.match(str(value)):
            raise RouteRejected(f"路由的 {key} 不是 64 位十六进制: {value!r}")
...
def main() -> int:
    ...
    parser.add_argument("--seasonal-period", type=int, default=None,
                        help="留空则从冻结 spec 的 seasonal_periods 取；若给出必须与注册值相等")
    ...
    if is_master_baseline:
        # 主控基线：与主控 numeric_baselines 同序（train/cal/dec/test）跑全量，
        # 再切测试段。**真值与特征在接口上分离**（第六轮修复 2）：
        #   features    = 仅含特征的推理视图（targets 一律 None）
        #   fit_targets = 仅 train/calibration 行有值，其余 NaN
        order = ("train", "calibration", "decision", "test")
        feats = {s: to_inference_bundle(bundle.segment(s)) for s in order}
        features = FeatureBundle(
            numeric_history=np.concatenate([feats[s].numeric_history for s in order]),
            semantic=np.concatenate([feats[s].semantic for s in order]),
            quality=np.concatenate([feats[s].quality for s in order]),
            text_available=np.concatenate([feats[s].text_available for s in order]),
            origin_index=np.concatenate([feats[s].origin_index for s in order]),
            targets=None, targets_standardized=None,
        )
        segments = np.concatenate([np.full(len(feats[s].numeric_history), s) for s in order])
        horizon_h = int(feats["train"].targets_standardized.shape[1]) \
            if feats["train"].targets_standardized is not None else None
        horizon_h = int(np.asarray(bundle.segment("train")["targets_standardized"]).shape[1])
        fit_targets = np.full((len(segments), horizon_h), np.nan)
        for s in ("train", "calibration"):
            mask = segments == s
            fit_targets[mask] = np.asarray(bundle.segment(s)["targets_standardized"], dtype=float)

        period = task_seasonal_period(args.split_spec, args.task)
        if args.seasonal_period is not None and int(args.seasonal_period) != period:
            print(f"seasonal-period={args.seasonal_period} 与冻结 spec 注册值 {period} 不符",
                  file=sys.stderr)
            return 3
        baseline = numeric_baselines(features, segments, fit_targets,
