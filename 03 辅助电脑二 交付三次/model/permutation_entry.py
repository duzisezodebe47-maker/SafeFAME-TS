...
def main() -> int:
    ...
    parser.add_argument("--split-spec-sha256", default=None,
                        help="主控冻结的 split_spec 哈希；提供则核对 manifest.inputs")
    ...
    # 半段按目标时间边界切（A.5），跨半段窗口剔除
    half_masks = decision_halves_by_target_time(bundle, "decision")
    halves = {name: decision.slice(np.flatnonzero(mask)) for name, mask in half_masks.items()}
    ...
    summary = {
        ...
        "decision_halves_rule": "按目标时间中点切；跨半段 H 窗口两边都不收",
